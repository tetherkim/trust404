...
import { check, hashShape, addressShape } from '../common/crypto.js';
import { creditStateAbi } from '../common/abi.js';
import { quantity } from '../common/rpc.js';

/**
 * Direct RPC Runtime Adapter.
 * Provides historical state replay for CreditState and local execution harness for Anvil.
 */
export class RpcRuntimeAdapter {
   ...
   *
   * @param {object} params
   * @param {string|number|bigint} params.blockNumber Historical block number
   * @param {string} [params.blockHash] Optional expected block hash
  ...
  async replayStateAtBlock({ blockNumber, blockHash, targetContract, subject }) {
    ...
    if (blockHash !== undefined) {
      check(block.hash === blockHash, 'REORG');
    }
  ...
  /**
   * Simulates, signs, and broadcasts anchorBatch transaction to local testnet.
   *
   * @param {object} params
  ...
  async stageAndBroadcast({ to, data, privateKey }) {
    ...
    check(
      ['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname) && this.chainId === 31337,
      'LOCAL_SIGNING_ONLY'
    );
    ...
    // Direct eth_call preflight simulation
