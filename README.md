...
로컬 체인은 거래 전송 직후 receipt가 아직 없을 수 있어 50ms 간격으로 확인합니다. 로컬 시연의 거래 확인은 5초, Anvil 시작 확인은 약 5초로 제한하며 테스트 취소 시 자식 프로세스를 정리합니다. `ExperimentalWarning: SQLite`는 Node의 기능 상태 안내이며 테스트 실패 자체는 아닙니다. 이 로컬 조회 설정은 Base Sepolia의 Aomi 실행에는 적용하지 않습니다.
...
| `demo/` | 로컬 체인 시연, AIM 무신뢰 검증, 감사 UI, 대출 포털 |
