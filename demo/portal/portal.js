...
async function refresh(){
 ...
 const cell=document.createElement('td');if(job.state==='done'){for(const [name,label] of [['evidence.json','단건 증거'],['verify-config.json','단건 검증 설정'],['audit.json','전체 감사 파일'],['execution.json','Aomi 이력'],['trust.json','신뢰 기준'],['profiles.json','서버 설정']]){const a=document.createElement('a');a.href=`/api/jobs/${job.id}/files/${name}`;a.download=name;a.textContent=label;cell.append(a);}}
