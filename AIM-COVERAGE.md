...
제품의 기준은 `../AIM.md`의 **오프체인 승인·거절 판단에 대한 검증 가능한 증거 생성**이다. 금액·통화·판단 규칙·체인·실행 도구는 시연 구현의 선택 사항이다. 실제 자금 이동은 하지 않는다.
...
| 기관 서버·DB 없는 검증 | 검증자는 공개 증거·신뢰 설정·체인 RPC만 사용 | `npm run demo:aim`: 기관 DB를 보존하면서 검증 프로세스의 읽기 권한 차단 |
...
**기관 DB를 없애는 것은 요건이 아니다. 검증자가 기관 DB에 접근하지 않아야 한다.** `demo:aim`은 기관 DB와 원본 자료를 유지하고, 별도 Node 검증 프로세스에 프로그램 코드·감사자에게 전달한 공개 파일의 읽기 권한만 허용한다. 기관 DB 읽기가 `ERR_ACCESS_DENIED`로 실패하는 것을 먼저 확인한 뒤, 같은 권한으로 실제 단건 검증과 전체 감사를 실행한다. 감사 중 파일 쓰기 권한도 필요 없다. 시연 종료 시에만 이 명령이 만든 임시 자료를 정리한다.
